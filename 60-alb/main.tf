...
module "records" {
  ...
  zone_name = var.zone_name #lingaiah.online
  records = [
    {
      name    = "roboshop-${var.environment}" # roboshop-dev.lingaiah.online
      type    = "A"
      alias   = {
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["roboshop-${var.environment}.${var.zone_name}"] #roboshop-dev.lingaiah.online
    }
  }
}
